...
CREATE INDEX operator_zone_zone_idx ON operator_zone (maps_to_zone_id);
...
CREATE INDEX permit_plate_idx ON permit (plate);
CREATE INDEX permit_validity_idx ON permit (valid_from, valid_to);
CREATE TRIGGER permit_touch
...
CREATE INDEX parking_session_plate_idx ON parking_session (plate);
CREATE INDEX parking_session_position_gix ON parking_session USING GIST (position);
CREATE INDEX parking_session_status_idx ON parking_session (status);
...
CREATE INDEX rule_evaluation_expires_idx ON rule_evaluation (expires_at);

CREATE TABLE fine_event (
  id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX fine_event_session_idx ON fine_event (session_id);
CREATE INDEX fine_event_issued_idx ON fine_event (issued_at);
...
CREATE INDEX notification_pending_idx ON notification (scheduled_at) WHERE dispatched_at IS NULL;
