...
CREATE INDEX IF NOT EXISTS regulation_effective_idx ON regulation (effective_from, effective_to);
CREATE OR REPLACE TRIGGER regulation_touch
...
CREATE INDEX IF NOT EXISTS rule_regulation_idx ON rule (regulation_id);
CREATE INDEX IF NOT EXISTS rule_kind_priority_idx ON rule (kind, priority DESC);
CREATE OR REPLACE TRIGGER rule_touch
...
CREATE TABLE IF NOT EXISTS rule_time_window (
  id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS rule_time_window_rule_idx ON rule_time_window (rule_id);
...
CREATE INDEX IF NOT EXISTS rule_applies_to_rule_idx ON rule_applies_to (rule_id);
CREATE INDEX IF NOT EXISTS rule_applies_to_target_idx ON rule_applies_to (target_kind, target_id);
